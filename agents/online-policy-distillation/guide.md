...
| $\pi_\theta(y\mid x)$ | 学生模型当前策略，参数为$\theta$ |
| $\pi_T(y\mid x)$ | 教师模型策略（可以是强模型、奖励模型、规则验证器、学生EMA版本） |
| $s_t = (x, y_{<t})$ | 第t步的上下文状态，包含输入和前t-1步生成的前缀 |
| $V$ | 词表大小 |
| $\pi_\theta(v\mid s_t)$ | 学生模型在状态$s_t$下生成token $v$的概率 |
| $\pi_T(v\mid s_t)$ | 教师模型在状态$s_t$下生成token $v$的概率 |
...
|------|---------|---------|---------|----------|------------|
| SFT | 最小化$- \log \pi_\theta(y^*_t\mid s_t^*)$ | 教师/人工生成的固定前缀$s_t^*$ | Token硬标签 | 高 | 极高 |
| 离线KD | 最小化$D(\pi_T(\cdot\mid s_t^*) \parallel \pi_\theta(\cdot\mid s_t^*))$ | 教师/人工生成的固定前缀$s_t^*$ | Token分布级 | 高 | 高 |
